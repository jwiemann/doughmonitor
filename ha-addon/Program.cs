...
var options = builder.Configuration.GetSection("Monitor")
    .Get<MonitorOptions>() ?? throw new InvalidOperationException("Missing 'Monitor' configuration section");
var supervisorToken = Environment.GetEnvironmentVariable("SUPERVISOR_TOKEN");
if (supervisorToken is not null)
{
    using var supervisorHttp = new HttpClient
    {
        BaseAddress = new Uri("http://supervisor")
    };
